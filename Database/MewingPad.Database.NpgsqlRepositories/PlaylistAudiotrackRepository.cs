...
public class PlaylistAudiotrackRepository(MewingPadDbContext context)
    : IPlaylistAudiotrackRepository
{
    private readonly MewingPadDbContext _context = context;

    private readonly ILogger _logger =
        Log.ForContext<PlaylistAudiotrackRepository>();
    ...
    )
    {
        ...
        try
        {
            await _context.PlaylistsAudiotracks.AddAsync(
                new(playlistId, audiotrackId)
            );
    ...
    public async Task DeleteByPlaylist(Guid playlistId)
    {
        ...
        try
        {
            var pairs = await _context
                .PlaylistsAudiotracks.Where(pa => pa.PlaylistId == playlistId)
                .ToListAsync();
            if (pairs.Count == 0)
            {
                return;
            }
            _context.PlaylistsAudiotracks.RemoveRange(pairs);
            await _context.SaveChangesAsync();
        }
        catch (Exception ex)
        {
    ...
    public async Task DeleteByAudiotrack(Guid audiotrackId)
    {
        ...
        {
            var pairs = await _context
                .PlaylistsAudiotracks.Where(pa =>
                    pa.AudiotrackId == audiotrackId
                )
                .ToListAsync();
            if (pairs.Count == 0)
            {
    ...
    public async Task<bool> IsAudiotrackInPlaylist(
        Guid audiotrackId,
        Guid playlistId
    )
    {
        ...
        try
        {
            inPlaylist = await _context.PlaylistsAudiotracks.AnyAsync(pa =>
                pa.AudiotrackId == audiotrackId && pa.PlaylistId == playlistId
            );
        }
        catch (Exception ex)
        {
