...
) : IScoreService
{
    ...
    public async Task CreateScore(Score score)
    {
        ...
        if (
            await _scoreRepository.GetScoreByPrimaryKey(
                score.AuthorId,
                score.AudiotrackId
            )
            is not null
        ...
        {
            _logger.Error(
                $"Score (AuthorId = {score.AuthorId}, AudiotrackId = {score.AudiotrackId}) already exists"
            );
            throw new ScoreExistsException(score.AuthorId, score.AudiotrackId);
        }
        await _scoreRepository.AddScore(score);
        _logger.Information(
            $"Score (AuthorId = {score.AuthorId}, AudiotrackId = {score.AudiotrackId}) added"
        );
    ...
    public async Task<Score> UpdateScore(Score score)
    {
        ...
        {
            _logger.Error(
                $"Score (AuthorId = {score.AuthorId}, AudiotrackId = {score.AudiotrackId}) not found"
            );
        ...
        _logger.Information(
            $"Score (AuthorId = {score.AuthorId}, AudiotrackId = {score.AudiotrackId}) updated"
        );
    ...
    {
        _logger.Verbose(
            $"Entering GetScoreByPrimaryKey({authorId}, {audiotrackId})"
        );

        var score = await _scoreRepository.GetScoreByPrimaryKey(
            authorId,
            audiotrackId
        );
        if (score is null)
        {
            _logger.Error(
                $"Score (AuthorId = {authorId}, AudiotrackId = {audiotrackId}) not found"
            );
